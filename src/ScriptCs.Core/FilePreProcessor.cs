...
namespace ScriptCs
{
    public class FilePreProcessor : IFilePreProcessor
    {
        ...
        public string ProcessFile(string path)
        {
            ...
            var parseResult = ParseFile(FileSystem, path);

            var result = GenerateRs(parseResult.Rs);
            if (!string.IsNullOrWhiteSpace(result))
            {
                result += FileSystem.NewLine;
            }
            ...
            return result.TrimEnd(FileSystem.NewLine.ToCharArray());
        }
        ...
        {
            return string.Join(FileSystem.NewLine, rLines.Distinct());
        }
